...
class APISpecificationEndpointHandler(contextPath: String) : AbstractEndpointHandler<OpenAPI>(
  ...
  private val apiSpecification by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
    // this is generated lazily upon first request because it may not be needed,
    // but also to allow all dynamic endpoints to be registered in container lifecycle callbacks
    ...
    OpenAPISpecificationBuilder(serializationFactory)
        ...
        .withServer(OpenAPI.Server(url = URL("http://localhost:8500")))
...
/**
 * This is a non-threadsafe single-use object.
 */
