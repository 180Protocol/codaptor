...
class APISpecificationEndpointHandler(contextPath: String) : AbstractEndpointHandler<OpenAPI>(
    ...
    mappingParameters = ContextMappedHandler.Parameters(contextPath, true)
), CordaptorComponent, LifecycleAware {

  private val serializationFactory: SerializationFactory by inject()

  override fun canHandle(exchange: HttpServerExchange): Boolean = exchange.requestMethod == Methods.GET

  private val urlBuilder by inject<URLBuilder>()
  private val notifications: NodeNotifications by inject()

  override fun onStarted() {
    val apiSpecUrl = urlBuilder.toAbsoluteUrl(mappingParameters.path)
  ...
  private val apiSpecification by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
    ...
    OpenAPISpecificationBuilder(serializationFactory)
        ...
        .withServer(OpenAPI.Server(url = URL(urlBuilder.baseUrl)))
  ...
  override fun doHandle(exchange: HttpServerExchange, subject: Subject) {
    sendResponse(exchange, Response(apiSpecification))
  }
}
